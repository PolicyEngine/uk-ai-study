...
the $+2.6$ per cent uplift. I interpret $\eta$ as an own-wage labour
demand elasticity; with 7 per cent of workers displaced the inflow
...
the own-wage labour demand elasticity applied to each destination
group's proportional labour-supply inflow. $\eta = 0$ reproduces the
central scenario exactly.}
\end{table}

The ripple channel loads almost entirely on the Exchequer. At $\eta =
...
margin. The distributional headlines barely move: BHC poverty rises by
only 0.19 points more than in the central run, and the Gini increase
actually \emph{eases} slightly, since the cuts fall on employed workers
concentrated in the middle and upper deciles. Excluding ripples
therefore makes the central fiscal estimate conservative, while the
poverty and inequality conclusions are robust to the omission.
