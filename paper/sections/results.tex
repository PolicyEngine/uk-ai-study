...
Table~\ref{tab:mixedwage} reports a reduced-form interpolation between the two endpoints. Moving from the all-wage calibration to all displacement reduces the Exchequer cost from \pounds 22.4 billion to \pounds 18.4 billion, but raises BHC poverty from 0.06 to 1.87 points and the Gini effect from $-0.37$ to $+1.10$ points. The half-index case costs \pounds 20.5 billion, raises poverty by 0.92 points and raises the Gini by 0.35 points. These changes cannot be attributed to the adjustment margin alone: after the common uplift, net employee-income changes are $-4.34$, $-4.43$, $-4.82$, $-5.07$ and $-5.32$ per cent as $\lambda$ rises, because the wage endpoint is calibrated to the wage bill whereas displacement is calibrated to employee headcount. Conditional on that interpolation, poverty and inequality move monotonically towards their displacement endpoints and the fiscal cost remains between \pounds 18.4 and \pounds 22.4 billion.
...
Scenario & Displaced & Exchequer cost & $\Delta$ Poverty & $\Delta$ Gini \\
 & (million) & (\pounds bn/yr) & BHC (pp) & (pp) \\
\midrule
\multicolumn{5}{l}{\emph{$\lambda$ scales the 7 per cent displacement-rate endpoint}} \\
$\lambda=0$ & 0.00 & 22.4 & +0.06 & $-$0.37 \\
$\lambda=.25$ & 0.40 & 20.5 & +0.45 & $-$0.03 \\
$\lambda=.50$ & 0.81 & 20.5 & +0.92 & +0.35 \\
$\lambda=.75$ & 1.21 & 19.3 & +1.43 & +0.73 \\
...
\vspace{0.4em}
\parbox{0.92\textwidth}{\footnotesize\emph{Note:} In the mixed panel, $\lambda$ scales the 7 per cent employee-displacement rate and $(1-\lambda)$ scales a cut equal to 7 per cent of the baseline wage bill. It retains the central 2.6 per cent complementarity-graded uplift and allocates cuts by C-AIOE. The endpoints therefore do not conserve the same realised earnings loss. The second panel holds the displaced workers and capital shock fixed and changes only the employment-weighted survivor-wage parameter. These are author-designed reduced-form comparative statics, not estimates of equilibrium wage responses.}
\end{table}
