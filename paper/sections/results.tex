...
As an exploratory spatial sensitivity, I project the central scenario onto the 650 Westminster parliamentary constituencies using PolicyEngine's calibrated constituency weight matrix. A major comparability limitation applies. The weight matrix indexes the \emph{enhanced} FRS 2023--24 dataset, on which occupation codes cannot be joined directly, so SOC major group is imputed from the plain-FRS distribution within age-band $\times$ gender $\times$ region $\times$ earnings-decile cells (98.9 per cent weighted coverage from the full four-way cell). The calibration contains no local occupation target: constituency variation therefore reflects demographic and earnings composition under one imputation and one seed-0 draw, not observed local occupation mixes. The displacement mask is drawn using national household weights obtained by summing the same 650 constituency-matrix rows used for every outcome, so the displacement, income and poverty columns now describe one common scenario without post-hoc rescaling. % results/geo/imputation_notes.json
...
Under this imputation, the realised national displacement rate is 6.72 per cent (67 workers per 1,000), close to the 7 per cent first-order target; average household income is nearly unchanged ($+0.1$ per cent), but the synthetic local projections vary widely. Regional income changes range from $-1.35$ per cent in Northern Ireland and about $-1.08$ per cent in London and Scotland to $+1.59$ per cent in the North East. Regional displacement spans 59 per 1,000 workers in Wales to 89 in Northern Ireland. Constituency income changes range from $-6.0$ to $+4.0$ per cent, and 407 of 650 constituencies gain on net where wage and capital gains outweigh displacement losses. These patterns are sensitive to the imposed imputation and single assignment draw rather than evidence of measured local occupational clustering. % results/geo/region_summary.csv, results/geo/constituency_impacts.csv
