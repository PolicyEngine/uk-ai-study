...
Everything so far conditions on exposure-proportional incidence. But allocation is at least as uncertain as shock size, so I reallocate the same central calibration across five families. Four are stylised: exposure-proportional, junior-concentrated, expertise-compression and uniform. The fifth is a Klein-anchored top-loaded stress test: exposure-proportional draws multiplied by an author-imposed junior tilt (1.29) and wage-tier tilt (9.6 above, 0.6 below median person earnings). It contains no geographic tilt. Table~\ref{tab:incidence} gives means and assignment-draw standard deviations over 20 paired seeds.
...
Klein-anchored top-loaded stress test & 1.61 & 33.3 $\pm$ 1.5 & +2.24 $\pm$ 0.17 & +2.40 & +0.96 $\pm$ 0.14 \\
\bottomrule
\end{tabular}

\vspace{0.4em}
\parbox{0.92\textwidth}{\footnotesize\emph{Note:} The Klein-anchored family combines exposure with author-imposed junior (1.29) and wage-tier (9.6 above / 0.6 below median person earnings) multipliers. No geographic tilt is applied: the December 2025 revision reports minimal, statistically insignificant London effects. The wage-tier values descend from press-release maximum-exposure scalings rather than the paper's per-standard-deviation estimates, so this is a stress test, not an estimated displacement schedule. $\pm$ values are assignment-draw standard deviations.}
\end{table}
...
Within these calibrations, who bears the shock determines what it costs. The mean Exchequer cost ranges from \pounds 14.9 billion under uniform incidence to \pounds 33.3 billion under the Klein-anchored stress test, generated by reallocating which workers lose their jobs. Top-loaded incidence is fiscally expensive because displaced high earners take large income tax and National Insurance payments with them. These are paired-seed scenario comparisons, not population confidence statements.
...
The Gini rises in all five simulated families---20-draw means of +0.96 to +1.13 percentage points---just as it rises in all 66 grid cells. I do not interpret the pairwise ordering. The directional result is explicitly conditional on these displacement calibrations.

\subsection{The adjustment margin: wage cuts versus displacement}
\label{sec:results-wagemargin}

The incidence families reallocate displacement across workers; the wage-margin family instead applies the headline 7 per cent parameter to a different base. Every employee keeps their job, and exactly 7 per cent of the baseline employee wage bill is removed through author-imposed occupation-level cuts proportional to C-AIOE or PSS. The central displacement row instead removes the earnings of a weighted 7 per cent of employees, so realised wage-bill losses need not be identical. Table~\ref{tab:wagemargin} compares these clearly labelled calibrations on the seed-0 basis.
