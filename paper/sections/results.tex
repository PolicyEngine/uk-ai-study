...
The wage channel, by contrast, is almost distributionally flat. Figure~\ref{fig:wages} shows the percentage change in employment income among workers who remain employed: gains run from 2.51 per cent in the bottom decile to 2.72 per cent in the top, a spread of barely 0.2 percentage points around the 2.6 per cent average uplift. Exposure is sufficiently widespread across the deciles in which people work that the productivity dividend is spread thinly and evenly. It is the displacement margin, not the wage margin, that drives the distributional results below. That allocation is a property of the scenario design rather than a finding: in the US evidence of \citet{acemoglu2022tasks}, task displacement operates chiefly through relative wage declines, which account for 50--70 per cent of changes in the wage structure. Section~\ref{sec:results-wagemargin} quantifies exactly this: a wage-margin implementation of the same aggregate shock shifts losses from benefit caseloads to earnings, with a comparable fiscal cost but an almost entirely different poverty and inequality profile.
...
The incidence families reallocate displacement across workers; the wage-margin family of Section~\ref{sec:incidence} instead changes the margin on which the same aggregate shock arrives. Section~\ref{sec:results-channels} flagged that the dominance of the displacement margin over the wage margin ``is a property of the scenario design rather than a finding''; this subsection quantifies what that design choice is worth. Every employee keeps their job, and the 7 per cent aggregate earnings loss of the central scenario is delivered instead as occupation-level percentage wage cuts --- proportional to the C-AIOE in one variant and to the potential-supply-shift (PSS) measure of \citet{hosseinilichtinger2026barriers} in the other --- with the wage uplift and capital channels unchanged. Table~\ref{tab:wagemargin} compares the two variants with the central displacement scenario on the same seed-0 basis; the wage-margin rows are deterministic, since with no displacement draw there is nothing to seed.

\begin{table}[htbp]
\centering
\caption{The same aggregate earnings shock on two adjustment margins (central calibration; displacement row single draw, seed 0; wage-margin rows deterministic)}
...
\midrule
Central displacement & 18.4 & +1.87 & +1.10 \\
Wage margin (C-AIOE gradient) & 22.4 & +0.03 & $-$0.37 \\
Wage margin (PSS gradient) & 22.2 & +0.07 & $-$0.29 \\
\bottomrule
\end{tabular}

\vspace{0.4em}
\parbox{0.92\textwidth}{\footnotesize\emph{Note:} All three scenarios remove 7 per cent of the aggregate employee wage bill and apply the 2.6 per cent wage uplift and the capital shock. The wage-margin scenarios displace no one: cuts are occupation-level percentages proportional to the stated gradient, calibrated so the aggregate earnings loss matches the central scenario. After-housing-costs poverty changes are +1.88, +0.17 and +0.25 points respectively.}
\end{table}

Three contrasts stand out. First, the fiscal cost does not fall when the shock moves to the wage margin --- it rises modestly, from \pounds 18.4 billion to \pounds 22.4 billion (\pounds 22.2 billion under the PSS gradient). The direction is consistent with the mechanics of the two margins: wage cuts shave earnings at each worker's marginal income tax and National Insurance rates, and the cuts fall disproportionately on higher-paid exposed occupations, whereas displacement removes whole earnings blocks --- taxed, in effect, at average rates --- and is partially offset on the spending side by means-tested benefits that reach only the minority of displaced workers in low-income households. The choice of adjustment margin therefore changes the composition of the Exchequer cost, not its broad magnitude: the displacement implementation is, if anything, a lower bound on the fiscal cost of a given aggregate earnings shock.

Second, the poverty effect all but disappears. The +1.87-point BHC poverty rise of the central scenario collapses to +0.03 points under the C-AIOE gradient and +0.07 under the PSS gradient (+0.17 and +0.25 after housing costs, against +1.88). The reason is arithmetic rather than subtle: the same aggregate loss is spread thinly across the whole employed population instead of being concentrated as total earnings losses on 1.6 million displaced workers, so almost no household is pushed across the (absolute) poverty line. The displacement implementation is correspondingly an upper bound on the within-year poverty effect of a given aggregate shock.

Third, the sign of the inequality effect flips. Where the Gini rises in every displacement cell and every incidence family, the wage-margin variants \emph{reduce} it, by 0.37 and 0.29 percentage points: percentage cuts scale with exposure, which is highest in well-paid professional and associate-professional occupations, so the wage-margin shock compresses the earnings distribution from above --- annual household losses run from \pounds 19 in the bottom decile to \pounds 3,035 in the top under the C-AIOE gradient (\pounds 44 to \pounds 2,850 under PSS), a smoother and less top-spiked gradient than the displacement profile, which mixes near-zero losses with deciles losing \pounds 3,700--4,600. This is the household-level counterpart of the equalising wage-adjustment channel in \citet{hosseinilichtinger2026barriers}, and it sharpens the reading of the paper's most robust displacement result: the universal Gini rise of Figure~\ref{fig:gridgini} is a property of the unemployment margin, not of the aggregate shock. What survives the change of margin is the fiscal conclusion --- roughly \pounds 20 billion a year leaves the public finances whichever way a 7 per cent earnings shock arrives --- while the poverty and inequality conclusions are properties of the assumption that the shock arrives as job loss.
